...
main() {
  ...
  install_dir="${GH_INSTALL:-"${ANY_INSTALL_ROOT}/.gh"}"
  ...
  asset="${temp_dir}/gh_2.54.0_linux_amd64.tar.gz"
  anyi_info "asset=${asset}"

  if anyi_check_command "gh auth status"; then
    gh release download "v2.54.0" --repo "https://github.com/cli/cli" --pattern "gh_2.54.0_linux_amd64.tar.gz" --output "${asset}"
  else
