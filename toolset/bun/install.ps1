...
function _any_install_main {
  ...
  function main {
    ...
    if (Test-Path env:BUN_INSTALL) { $install_dir = (Get-Item env:BUN_INSTALL).Value } else { $install_dir = "${Env:ANY_INSTALL_ROOT}/.bun" }
    ...
    $asset="${temp_dir}/bun-windows-x64.zip"
    anyi_info "asset=${asset}"
    
    if (anyi_check_command "gh auth status") {
      gh release download --repo "https://github.com/oven-sh/bun" --pattern "bun-windows-x64.zip" --output "${asset}"
    } else {
