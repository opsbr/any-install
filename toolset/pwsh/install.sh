...
main() {
  ...
  install_dir="${PWSH_INSTALL:-"${ANY_INSTALL_ROOT}/.pwsh"}"
  ...
  case "${os}-${arch}" in
    "linux-x64")
      ...
      asset="${temp_dir}/powershell-7.4.5-linux-x64.tar.gz"
      anyi_info "asset=${asset}"

      if anyi_check_command "gh auth status"; then
        gh release download "v7.4.5" --repo "https://github.com/PowerShell/PowerShell" --pattern "powershell-7.4.5-linux-x64.tar.gz" --output "${asset}"
      else
    ...
    "macos-arm64")
      ...
      asset="${temp_dir}/powershell-7.4.5-osx-arm64.tar.gz"
      anyi_info "asset=${asset}"

      if anyi_check_command "gh auth status"; then
        gh release download "v7.4.5" --repo "https://github.com/PowerShell/PowerShell" --pattern "powershell-7.4.5-osx-arm64.tar.gz" --output "${asset}"
      else
        anyi_download "${url}" "${asset}"
      fi
