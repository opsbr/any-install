...
function _any_install_main {
  ...
  function main {
    ...
    if (Test-Path env:SHELLCHECK_INSTALL) { $install_dir = (Get-Item env:SHELLCHECK_INSTALL).Value } else { $install_dir = "${Env:ANY_INSTALL_ROOT}/.shellcheck" }
    ...
    $asset="${temp_dir}/shellcheck-stable.zip"
    anyi_info "asset=${asset}"
    
    if (anyi_check_command "gh auth status") {
      gh release download "stable" --repo "https://github.com/koalaman/shellcheck" --pattern "shellcheck-stable.zip" --output "${asset}"
    } else {
