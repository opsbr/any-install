...
main() {
  ...
  install_dir="${SHELLCHECK_INSTALL:-"${ANY_INSTALL_ROOT}/.shellcheck"}"
  ...
  asset="${temp_dir}/shellcheck-stable.linux.x86_64.tar.xz"
  anyi_info "asset=${asset}"

  if anyi_check_command "gh auth status"; then
    gh release download "stable" --repo "https://github.com/koalaman/shellcheck" --pattern "shellcheck-stable.linux.x86_64.tar.xz" --output "${asset}"
  else
